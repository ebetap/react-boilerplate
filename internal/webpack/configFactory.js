...
export default function webpackConfigFactory(buildOptions) {
  ...
  let webpackConfig = {
    ...
    plugins: removeNil([
      ...
      ifProdClient(() =>
        new webpack.optimize.CommonsChunkPlugin({
          name: 'vendor-splitted-alpha',
          minChunks: ({ resource }) => {
            const chuckConfig = [
              ...config('nodeModulesSpecialChuck').alpha,
              ...config('nodeModulesSpecialChuck').beta,
              ...config('nodeModulesSpecialChuck').omega,
            ];
            if (!chuckConfig) return;
            return chuckConfig.reduce((acc, val) => {
              if (resource.indexOf(val) >= 0) return true;
              return acc;
            }, false);
          },
        })),
      ...
      ifProdClient(() =>
        new webpack.optimize.CommonsChunkPlugin({
          name: 'vendor-splitted-omega',
          minChunks: ({ resource }) => {
            const chuckConfig = config('nodeModulesSpecialChuck').omega;
            if (!chuckConfig) return;
            return chuckConfig.reduce((acc, val) => {
              if (resource.indexOf(val) >= 0) return true;
              return acc;
